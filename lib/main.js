...
const erlangLsDowloadSize = 297035
...
class ErlangLanguageClient extends AutoLanguageClient {
    ...
    startServerProcess () {
        ...
        const erlangLsWorkingDir = path.join(erlangLsHome, "erlang_ls-" + erlangLsVersion)
        const erlangLsErlScript = path.join(erlangLsWorkingDir, erlangLsErlScriptRelPath)
        const erlangLsLogDir = path.join(erlangLsHome, 'log')
        ...
        return this.fileExists(erlangLsErlScript)
            .catch(() => {
                ...
                return this.fileExists(erlangLsHome)
                    ...
                    .then(() => {
                        ...
                        return DownloadFile(erlangLsDownloadUrl, erlangLsLocalPath, (_bytesDone, percent) => {
                            this.updateStatusBar("Downloading server file: " + percent + "%")
                        }, erlangLsDowloadSize)
                    })
                    .then(() => {
                        ...
                        return new Promise((resolve, reject) => {
                            ...
                            buildProcess.on('close', code => {
                                if (code == 0) {
                                    ...
                                    reject("Error building server (" + code + "): " + lastError)
                                }
                            })
                        })
                    })
            ...
            .then(() => {
                return new Promise(resolve => {
                    ...
                    const server = cp.spawn(erlangLsErlScript, ["-tstdio", "-ldebug", "-d" + erlangLsLogDir], { cwd:erlangLsWorkingDir })
                    server.stderr.on('data', (data) => console.error(data))
                    server.on('close', code => this.updateStatusBar("Server stopped (" + code + ")"))
